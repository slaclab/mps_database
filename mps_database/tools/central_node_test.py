...
class App:
  '''
  This class simulates the update message generated by an application card
  '''
  app=None
...
class Simulator:
  '''
  This class simulates update messages from all application cards.
  '''
  databaseFileName=""
  session=0
...
#    self.apps=[]
  ...
  def get_link_node(self, link_node_name):
  ...
  def get_all_link_node_devices(self, link_node_name, device_type):
    link_node = self.get_link_node(link_node_name)
    if (link_node == None):
      return []

    ln_crate_id = link_node.crate.id
    devices = []
    for c in link_node.crate.cards:
      if (device_type == 'digital'):
        devices += [ d for d in c.devices if d.is_digital() ]
      else:
        devices += [ d for d in c.devices if d.is_analog() ]

    return devices

  def get_all_link_node_analog_devices(self, link_node_name):
    print('hello')
    link_node = self.get_link_node(link_node_name)
    if (link_node == None):
      return []
...
class Tester:
  '''
  This class sends update messages containing a known state and
  verifies whether the central node mitigation response is as
  expected.
  '''
  simulator = None
  sock = None
  ...
  def test_link_node(self, link_node_name, device_type):
    '''
    Test the digital or analog devices attached to a link node.

    :type link_node_name: string (name of the link node sioc, e.g. sioc-gunb-mp01)
    :type device_type: string ('analog' or 'digital')
    :rtype: [ devices ]
    '''
    if (device_type == 'analog'):
      self.digital_device = None  
    devices = self.simulator.get_all_link_node_devices(link_node_name, device_type)
    for device in devices:
      self.test(device.id, device_type)
...
parser.add_argument('--host', metavar='hostname', type=str, nargs=1, help='machine where test server is running')
parser.add_argument('--port', metavar='size', type=int, nargs='?', help='server port (default=4356)')
parser.add_argument('--debug', metavar='level', type=int, nargs='?', help='set debug level output (default level 0)')
parser.add_argument('--report', action="store_true", help='generate pdf report')
parser.add_argument('--delay', action="store_true", help='add 1 second delay between tests')
parser.add_argument('--analog', action="store_true", help='test analog device')

# Either specify a device by its ID, or a link node name - if none, then test will run through all inputs (could be a lot!)
group_device = parser.add_mutually_exclusive_group()
group_device_specific = group_device.add_mutually_exclusive_group()
group_device_specific.add_argument('--device', metavar='device', type=int, nargs='?',
                                   help='device id (default - test all digital devices)')

group_device_ln = group_device.add_mutually_exclusive_group()
group_device_ln.add_argument('--ln', metavar='link node name (sioc)', type=str, nargs='?',
                             help='name of the link node sioc with the inputs to test '\
                               '(only test digital inputs by default, please use '\
                               '--analog option for testing analog devices.')
...
if args.report:
  report = True
  if (args.ln):
    docbook = DocBook('{}-{}-{}-report.xml'.\
                        format(args.database[0].name.split('/')[len(args.database[0].name.split('/'))-1].split('.')[0],
                               args.ln, device_type))
  else:
...
if args.ln:
  t.test_link_node(args.ln, device_type)
